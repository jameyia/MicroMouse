...
int adc_mr, adc_ml, adc_fl, adc_fr; // ADC value of left sensor looking front, right sensor looking front etc...
int hasleftwall, hasrightwall, hasfrontwall;
int turn_counter = 0; // This should only be updated when eiter turn_left or turn_right are called.
int left_upper = 0;
int left_lower = 0;
int right_upper = 0; 
int right_lower = 0;
...
// MOTOR
int direction_left = FORWARD ;
int direction_right = FORWARD;
int left_base_speed = DEFAULTSPEED;
int right_base_speed = DEFAULTSPEED;
...
/*******************************/
/* This handles the interrups  */
/* TIMER0: ADC, PID, ALGORITHM */
/* TIMER1: LEFTMOTOR           */
/* TIMER2: RIGHTMOTOR          */
/*******************************/
void interrupt isr( )
{
   //Timer0
    if(INTCONbits.TMR0IF == 1 && stop1 == 0)
    {
        INTCONbits.TMR0IF = 0;
        ...
        /*   Sensor 0    */
        /*****************/
        ADCON0bits.CHS = 0b0000; // Select channel 0 (AN0)
        ADCON0bits.GO = 1; //Start conversion
        while(ADCON0bits.GO == 1); //Wait until ADCON0bits.CHS0 = 0b0000; // Select channel 0 (AN0)l conversion is complete
                adc_mr = (ADRESH << 8) + ADRESL;

        hasrightwall = sensor(0, adc_mr);
        if(hasrightwall)
        {
            PORTDbits.RD0 = 1;
            wait = 0;
       ...
       while(ADCON0bits.GO == 1); //Wait until complete
            adc_fr = (ADRESH << 8) + ADRESL;

        hasfrontwall = sensor(1, adc_fr);
        if(hasfrontwall)
                PORTDbits.RD2 = 1;
        ...
        /*   Sensor 2    */
        /*****************/
       ADCON0bits.CHS = 0b0010; // Select channel 2 (AN2)
       ADCON0bits.GO = 1; //Start conversion
       while(ADCON0bits.GO == 1); //Wait until complete
            adc_ml = (ADRESH << 8) + ADRESL;

       hasleftwall = sensor(2, adc_ml);
       if(hasleftwall)
       {
           PORTCbits.RC4 = 1;
           wait = 0;
       ...
       else
       {
        ...
        /*   Sensor 3    */
        /*****************/
       ADCON0bits.CHS = 0b0011; // Select channel 3 (AN3)
       ADCON0bits.GO = 1; //Start conversion
       while(ADCON0bits.GO == 1); //Wait until complete
            adc_fl = (ADRESH << 8) + ADRESL;

       hasfrontwall = sensor(3, adc_fl);
       if(hasfrontwall)
           PORTCbits.RC6 = 1;
    ...
    if(PIR1bits.TMR1IF == 1)
    {
        ...
        set_left_speed(left_base_speed, &left_upper, &left_lower);
     ...
     if(PIR2bits.TMR3IF == 1)
    {
        ...
        set_right_speed(right_base_speed, &right_upper, &right_lower);

        TMR3H = right_upper;           //Preload at 25356
        TMR3L = right_lower;
